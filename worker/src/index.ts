...
const ALLOWED_ORIGINS = ['http://localhost:5173', 'https://devopsquest.pages.dev']
...
async function updateProgress(request: Request, env: Env): Promise<Response> {
  ...
  try {
    const body = await request.json<Partial<UserProgress>>()

    const existingProgress = await env.PROGRESS.get<UserProgress>(`progress:${userId}`, 'json')
