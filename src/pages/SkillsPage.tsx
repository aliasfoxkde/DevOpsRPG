    ...
    <div className="container mx-auto px-4 py-8 max-w-6xl">
      <div className="text-center mb-8">
        ...
        <p className="text-slate-400">
          Spend skill points to unlock abilities and enhance your powers
        </p>
      </div>
        ...
        <div className="bg-slate-800/80 rounded-xl border border-green-600/50 p-4 mb-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
            {activeBonuses.map((bonus, bonusIdx) => (
              <div
                key={bonusIdx}
                className="bg-green-900/30 rounded-lg p-2 border border-green-700/30"
              >
            ...
            {recommended && (
              <div className="mb-4 p-2 bg-amber-900/20 rounded-lg border border-amber-600/30 flex items-center gap-2">
                ...
                <span className="text-sm text-amber-200">
                  Recommended: Level up <strong>{recommended.name}</strong> next
                </span>
              </div>
            )}
            ...
            <div className="relative">
              ...
              <div className="space-y-3">
                {tree.skills.map((skill) => {
                  ...
                  const xpProgress = isMaxed
                    ? 100
                    : Math.round(
                        ((xp - XP_THRESHOLDS[level]) /
                          (XP_THRESHOLDS[level + 1] - XP_THRESHOLDS[level])) *
                          100,
                      )
                  ...
                  return (
                    <div key={skill.id} className="relative flex items-start gap-4">
                      {/* Connection dot */}
                      <div
                        className={`relative z-10 w-4 h-4 rounded-full mt-4 ${
                          level > 0
                            ? 'bg-amber-500'
                            : requirementsMet
                              ? 'bg-blue-500'
                      ...
                      >
                        <div className="flex items-start justify-between">
                          <div className="flex items-start gap-3">
                            {/* Skill Icon */}
                            <div
                              className={`text-3xl ${!requirementsMet ? 'grayscale opacity-50' : isMaxed ? '' : currentLevel > 0 ? '' : 'grayscale opacity-50'}`}
                            >
                              {skill.icon}
                            </div>
                            ...
                            <div className="flex-1">
                              ...
                              <div className="flex items-center gap-2 mb-1">
                                <h3
                                  className={`font-bold ${isMaxed ? 'text-amber-400' : 'text-white'}`}
                                >
                                  {skill.name}
                                </h3>
                                {isMaxed && (
                                  ...
                                  {Array.from({ length: skill.maxLevel }).map((_, i) => (
                                    <div
                                      ...
                                      className={`w-3 h-3 rounded-sm ${
                                        i < level ? 'bg-amber-500' : 'bg-slate-700'
                                      }`}
                                    />
                                  ))}
                                </div>
                          ...
                          <div className="ml-4">
                            {isMaxed ? (
                              <div className="text-amber-400 text-sm font-bold px-3 py-1">MAX</div>
                            ) : !requirementsMet ? (
                              <div className="text-slate-500 text-xs px-2 py-1">🔒</div>
                            ) : (
                              <button
      ...
      <div className="mt-8 bg-slate-800/80 rounded-xl border border-slate-700 p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
          {Object.entries(game.skillXp).map(([techId, xp]) => {
            ...
            const progressPercent = Math.min(100, (xp / 1000) * 100)
            return (
              <div
                key={techId}
                className="bg-slate-700/50 p-3 rounded-lg border border-slate-600/30"
              >
      ...
      <div className="mt-8 bg-slate-800/80 rounded-xl border border-slate-700 p-6">
        ...
        <ul className="text-sm text-slate-400 space-y-2">
          ...
          <li className="flex items-start gap-2">
            <span className="text-amber-400 mt-0.5">•</span>
            <span>
              Each skill can be leveled up to its max level (some skills have prerequisites)
            </span>
          </li>
          <li className="flex items-start gap-2">
            <span className="text-amber-400 mt-0.5">•</span>
