...
describe('ChallengesPage', () => {
  ...
  it('renders the challenges header, tabs and daily dash panel', () => {
    ...
    expect(
      screen.getByRole('heading', { level: 1, name: /Challenges/ }),
    ).toBeInTheDocument()
    expect(screen.getByRole('button', { name: '⚡ Daily Dash' })).toBeInTheDocument()
    expect(
      screen.getByRole('button', { name: '📅 Weekly Challenges' }),
    ).toBeInTheDocument()
    expect(
      screen.getByRole('button', { name: '🗓️ Monthly Challenges' }),
    ).toBeInTheDocument()
    ...
    expect(
      screen.getByRole('button', { name: /START DAILY DASH/ }),
    ).toBeInTheDocument()
  ...
  it('starts a daily dash run and shows the live timer', async () => {
    ...
    expect(
      screen.getByText('Complete quests to track them here!'),
    ).toBeInTheDocument()
  ...
  it('tracks real progress when a finished challenge is claimed', async () => {
    ...
    const save = JSON.parse(localStorage.getItem(STORAGE_KEYS.GAME)!)
    ...
    expect(
      within(closestContainer(screen.getByText('XP Harvest'), '.bg-card'))
        .getAllByText('✓').length,
    ).toBeGreaterThan(0)
    expect(
      within(closestContainer(screen.getByText('XP Harvest'), '.bg-card')).queryByRole(
        'button',
        { name: 'CLAIM!' },
      ),
