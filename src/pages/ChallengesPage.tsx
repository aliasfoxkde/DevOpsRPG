...
const WEEKLY_CHALLENGES: Challenge[] = [
  {
    id: 'weekly_quest_crusader',
    title: 'Quest Crusader',
    description: 'Complete 20 quests this week',
    icon: '⚔️',
    requirement: { type: 'complete_quests', count: 20 },
    rewards: { xp: 600, gold: 250, badge: 'weekly_crusader' },
  },
  {
    id: 'weekly_xp_harvest',
    title: 'XP Harvest',
    description: 'Earn 3000 XP this week',
    icon: '💎',
    requirement: { type: 'earn_xp', count: 3000 },
    rewards: { xp: 400, gold: 200 },
  },
  {
    id: 'weekly_streak_sentinel',
    title: 'Streak Sentinel',
    description: 'Maintain a 7-day streak',
    icon: '🔥',
    requirement: { type: 'maintain_streak', count: 7 },
    rewards: { xp: 500, gold: 200, badge: 'streak_sentinel' },
  },
  {
    id: 'weekly_minigame_champion',
    title: 'Mini-Game Champion',
    description: 'Complete 8 mini-games this week',
    icon: '🎮',
    requirement: { type: 'minigame', count: 8 },
    rewards: { xp: 350, gold: 150 },
  },
  {
    id: 'weekly_quiz_wizard',
    title: 'Quiz Wizard',
    description: 'Pass 15 quizzes this week',
    icon: '🧙',
    requirement: { type: 'quiz', count: 15 },
    rewards: { xp: 450, gold: 225, badge: 'quiz_wizard' },
  },
]

const MONTHLY_CHALLENGES: Challenge[] = [
  {
    id: 'monthly_devops_champion',
    title: 'DevOps Champion',
    description: 'Complete 100 quests this month',
    icon: '🏆',
    requirement: { type: 'complete_quests', count: 100 },
    rewards: { xp: 2500, gold: 1000, badge: 'devops_champion' },
  },
  {
    id: 'monthly_xp_legend',
    title: 'XP Legend',
    description: 'Earn 15000 XP this month',
    icon: '👑',
    requirement: { type: 'earn_xp', count: 15000 },
  ...
  },
  {
    ...
    icon: '🔥',
    requirement: { type: 'maintain_streak', count: 30 },
    rewards: { xp: 2000, gold: 1000, badge: 'monthly_master' },
  },
  {
    id: 'monthly_game_enthusiast',
    title: 'Game Enthusiast',
    description: 'Complete 40 mini-games this month',
    icon: '🎲',
    requirement: { type: 'minigame', count: 40 },
    rewards: { xp: 1200, gold: 600 },
  },
  {
    id: 'monthly_quiz_oracle',
    title: 'Quiz Oracle',
    description: 'Pass 75 quizzes this month',
    icon: '🔮',
    requirement: { type: 'quiz', count: 75 },
    rewards: { xp: 1800, gold: 900, badge: 'quiz_oracle' },
  },
]
...
export default function ChallengesPage() {
  const { game, addXP, addGold, grantBadge, incrementStat, startDailyDash, abandonDailyDash } =
    useGame()
  const [activeTab, setActiveTab] = useState<'daily' | 'weekly' | 'monthly'>('daily')
  const [challengeStates, setChallengeStates] = useState<Partial<Record<string, ChallengeState>>>(
    {},
  )
  ...
  useEffect(() => {
    const startTime = game.dailyDash.startTime
    if (!game.dailyDash.active || !startTime) {
      return
    }
    const interval = setInterval(() => {
      setDashTimeElapsed(Math.floor((Date.now() - startTime) / 1000))
    }, 1000)
    return () => {
  ...
  const dailyDashQuests = useMemo(() => {
    ...
    return game.dailyDash.completedQuests
      .map((qId) => allQuests.find((q) => q.id === qId))
      .filter(Boolean)
  ...
  const challengesWithProgress = useMemo(() => {
    return activeChallenges.map((c) => {
      ...
      const savedState = challengeStates[stateKey] || {
        progress: 0,
        completed: false,
        claimed: false,
      }
  ...
  const overallProgress = useMemo(() => {
    ...
    const completedQuests = challengesWithProgress.reduce((sum, c) => {
      if (c.requirement.type === 'complete_quests')
        return sum + Math.min(c.progress, c.requirement.count)
      return sum
    }, 0)
  ...
  const handleClaim = (challenge: (typeof challengesWithProgress)[0]) => {
    ...
    setChallengeStates((prev) => ({
      ...prev,
      [stateKey]: {
        ...(prev[stateKey] ?? { progress: 0, completed: false, claimed: false }),
        claimed: true,
      },
    }))
  }
    ...
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      <div className="text-center mb-8">
        ...
        <p className="text-slate-400">
          Complete weekly and monthly challenges to earn epic rewards!
        </p>
      </div>
        ...
        <div className="bg-gradient-to-br from-red-900/30 to-orange-900/30 rounded-xl border border-red-500/30 p-6 mb-8">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="text-right">
              ...
              <div
                className={`text-2xl font-bold ${game.dailyDash.bestTime !== null ? 'text-yellow-400' : 'text-slate-500'}`}
              >
          ...
          {!game.dailyDash.active &&
            game.dailyDash.bestTime !== null &&
            game.dailyDash.completedQuests.length > 0 && (
              <div className="text-center">
                <div className="text-green-400 font-bold mb-2">🎉 Last Dash Complete!</div>
                <div className="text-slate-400 text-sm">
                  {game.dailyDash.completedQuests.length} quests in{' '}
                  {formatTime(
      ...
      {activeTab !== 'daily' && (
        <div className="bg-card rounded-xl border border-border p-6 mb-8">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-sm text-slate-400">{formatTimeRemaining(expiresAt)}</div>
          </div>
          <div className="mb-4">
            ...
            <div className="h-4 bg-slate-800 rounded-full overflow-hidden">
              <div
                ...
                style={{
                  width: `${Math.min(100, (overallProgress.completed / overallProgress.total) * 100)}%`,
                }}
              />
            </div>
          </div>
        ...
        <div className="grid gap-4">
          {challengesWithProgress.map((challenge) => {
            const progressPercent = Math.min(
              100,
              Math.round((challenge.progress / challenge.requirement.count) * 100),
            )
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div
                    className={`w-14 h-14 rounded-xl flex items-center justify-center text-3xl ${
                      challenge.claimed ? 'bg-green-900/50' : 'bg-slate-800'
                    }`}
                  >
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-4 text-sm">
                      ...
                      {challenge.rewards.badge && <span className="text-blue-400">+ Badge</span>}
                    </div>
                  </div>
