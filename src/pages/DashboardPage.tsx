...
interface Achievement {
  ...
  name: string
  description: string
  /** Direct predicate on the completed topic count; when absent, use `condition`. */
  check?: (completedTopics: number) => boolean
  condition?: 'xp' | 'level' | 'streak' | 'topics'
  value?: number
}
...
function checkAchievement(
  key: string,
  progress: { xp: number; level: number; streakDays: number; completedTopics: number },
): boolean {
  ...
  switch (a.condition) {
    ...
    case 'xp':
      return progress.xp >= (a.value ?? 0)
    /* istanbul ignore next */
    case 'level':
      return progress.level >= (a.value ?? 0)
    /* istanbul ignore next */
    case 'streak':
      return progress.streakDays >= (a.value ?? 0)
    /* istanbul ignore next */
    case 'topics':
      return progress.completedTopics >= (a.value ?? 0)
    /* istanbul ignore next */
    default:
      return false
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <section aria-label="Statistics" className="grid md:grid-cols-3 gap-6 mb-8">
        ...
        >
          <h3 id="xp-heading" className="text-sm font-medium text-slate-400 mb-2">
            Total XP
          </h3>
          <p className="text-4xl font-bold text-blue-400" aria-live="polite">
          ...
          <div className="mt-3">
            ...
            <ProgressBar
              ...
              size="sm"
              color="primary"
              aria-label={`Level ${character.level} progress: ${xpProgress} of 100 XP`}
            />
        ...
        >
          <h3 id="streak-heading" className="text-sm font-medium text-slate-400 mb-2">
            Current Streak
          </h3>
        ...
        >
          <h3 id="progress-heading" className="text-sm font-medium text-slate-400 mb-2">
            Topics Completed
          </h3>
      ...
      <section className="mb-8" aria-labelledby="achievements-heading">
        <h2 id="achievements-heading" className="text-xl font-semibold mb-4 text-slate-100">
          Achievements
        </h2>
        <div
        ...
        >
          {earnedAchievements.length > 0 ? (
            earnedAchievements.map((achievement) => (
              ...
              >
                <div
                  className="w-12 h-12 bg-amber-500/20 rounded-full flex items-center justify-center text-xl"
                  aria-hidden="true"
                >
                  🏆
                </div>
      ...
      <section className="mb-8" aria-labelledby="level-progress-heading">
        <h2 id="level-progress-heading" className="text-xl font-semibold mb-4 text-slate-100">
          Progress to Next Level
        </h2>
        <div className="bg-slate-800 rounded-xl border border-slate-700 p-6">
          <div
            className="flex justify-between mb-2 text-sm text-slate-400"
            role="status"
            aria-live="polite"
          >
          ...
          />
          <p className="text-center text-slate-400 mt-2 text-sm">{xpProgress} / 100 XP</p>
        </div>
      </section>

      {/* Continue Learning */}
      <section aria-labelledby="continue-heading">
        <h2 id="continue-heading" className="text-xl font-semibold mb-4 text-slate-100">
          Continue Learning
        </h2>
        <Link to="/learn" className="btn btn-primary inline-block">
          Go to Learning Path
        </Link>
      </section>
    </div>
