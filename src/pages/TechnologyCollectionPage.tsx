...
import {
  ...
  CATEGORY_NAMES,
  type TechnologyCard,
} from '../data/technologyCollection'
...
export default function TechnologyCollectionPage() {
  ...
  const getTechCompletion = (
    techId: string,
  ): { completed: number; total: number; percentage: number } => {
    const techQuests = allQuests.filter(
      (q) => q.technologyId.toLowerCase() === techId.toLowerCase(),
    )
  ...
  const cardsByCategory = new Map<string, TechnologyCard[]>()
  ...
  const completedCards = TECHNOLOGY_COLLECTION.filter(
    (card) => getCardState(card) === 'completed',
  ).length
  const inProgressCards = TECHNOLOGY_COLLECTION.filter(
    (card) => getCardState(card) === 'in_progress',
  ).length
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400">
            Complete quests to unlock technology cards and build your collection!
          </p>
        </div>
          ...
          <div className="flex justify-around text-center">
            ...
            <div>
              <div className="text-3xl font-bold text-slate-400">
                {totalCards - completedCards - inProgressCards}
              </div>
        ...
        <div className="space-y-8">
          {[...cardsByCategory.entries()].map(([category, cards]) => (
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-center mb-2">
                        <div
                          className={`text-4xl mb-2 ${state === 'locked' ? 'grayscale opacity-50' : ''}`}
                        >
                          {card.icon}
                        </div>
                        <h3
                          className={`font-bold text-sm ${state === 'completed' ? 'text-amber-400' : 'text-slate-200'}`}
                        >
                          {card.name}
                        </h3>
                        <span
                      ...
                      <div className="mt-3">
                        <div className="flex justify-between text-xs text-slate-400 mb-1">
                          <span>Progress</span>
                          <span>
                            {completed}/{card.requiredQuests} quests
                          </span>
                        </div>
