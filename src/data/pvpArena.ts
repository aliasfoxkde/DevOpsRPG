...
export const PVP_QUESTIONS: Record<string, PVPQuestion[]> = {
  git: [
    {
      ...
      options: ['git stage', 'git commit', 'git push', 'git add'],
      correctIndex: 3,
      topic: 'Git',
    },
    {
      question: 'Which command creates a new branch?',
      options: ['git branch new', 'git checkout -b new', 'git new branch', 'git split'],
      correctIndex: 1,
      topic: 'Git',
    },
    {
      question: 'What does git merge do?',
      options: ['Deletes branches', 'Combines branch histories', 'Creates tags', 'Stages files'],
      correctIndex: 1,
      topic: 'Git',
    },
  ],
  docker: [
    ...
    {
      question: 'What is a Dockerfile instruction for running commands during build?',
    ...
    },
    {
      ...
      options: [
        'Builds single images',
        'Orchestrates multi-container apps',
        'Monitors containers',
        'Manages networks',
      ],
      correctIndex: 1,
  ...
  kubernetes: [
    {
      question: 'What is the basic deployable unit in K8s?',
      options: ['Pod', 'Node', 'Cluster', 'Service'],
      correctIndex: 0,
      topic: 'Kubernetes',
    },
    {
      question: 'What does a Service in K8s provide?',
      options: ['Storage', 'Network abstraction', 'CPU resources', 'Memory'],
      correctIndex: 1,
      topic: 'Kubernetes',
    },
    {
      question: 'What command lists K8s pods?',
      options: [
        'kubectl get pods',
        'kubectl list pods',
        'kubectl show pods',
  ...
  aws: [
    ...
    {
      question: 'What service provides serverless functions?',
  ...
  terraform: [
    {
      ...
      options: ['Container orchestration', 'Infrastructure as Code', 'Monitoring', 'CI/CD'],
      correctIndex: 1,
      topic: 'Terraform',
    },
    {
      question: 'What command initializes a Terraform working directory?',
      options: ['terraform start', 'terraform init', 'terraform new', 'terraform setup'],
      correctIndex: 1,
      topic: 'Terraform',
    },
    {
      question: 'What does terraform plan do?',
      options: ['Applies changes', 'Shows execution plan', 'Destroys resources', 'Formats code'],
      correctIndex: 1,
      topic: 'Terraform',
    },
  ],
  cicd: [
    ...
    },
    {
      ...
      options: ['A physical pipe', 'Automated workflow', 'Debugging tool', 'Testing framework'],
      correctIndex: 1,
      topic: 'CI/CD',
    },
  ],
...
export function generatePVPMatch(playerPoints: number): PVPMatch {
  ...
  const opponentRankIndex =
    minOpponentIndex + Math.floor(Math.random() * (maxOpponentIndex - minOpponentIndex + 1))
